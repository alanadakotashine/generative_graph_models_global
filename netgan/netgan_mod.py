"""
Modified version of 
the implementation of the method proposed in the paper:
...
NetGAN modified to include option to train generator without LSTM state. 
Training modified to use new stopping criteria.
"""
import sys
import os
...
class NetGAN:
    ...
    def __init__(self, N, rw_len, walk_generator, generator_layers=[40], discriminator_layers=[30],
        ...
        if wstate == 1:
            ...
            self.generator_function = self.generator_recurrent_zero_memory
    ...
    '''NEW GENERATORS'''
    def generator_recurrent_zero_memory(self, n_samples, reuse=None, z=None, gumbel=True, legacy=False):
        """
        Generate random walks using LSTM.
        Parameters
        ----------
        ...
        with tf.variable_scope('Generator') as scope:
            ...
            for i in range(self.rw_len):
                ...
                '''SET STATE TO ZERO'''
                state = (tf.contrib.rnn.LSTMStateTuple(tf.zeros_like(state[0].h), tf.zeros_like(state[0].c)),)
    ...
    def train(self, A_orig, val_ones, val_zeros,  max_iters=50000, stopping=None, eval_transitions=15e6,
              ...
              model_name=None, continue_training=False, path = '../', start_iter=0):
        """
        Parameters
        ----------
        ...
        if stopping == -1:  # use VAL criterion
            best_performance_val = 0.0
            patience_val = max_patience
            print("**** Using VAL criterion for early stopping ****")
        elif stopping == -2: #use VAL + SPEC criterion
            best_performance_auc = 0.0
            best_performance_ap = 0.0
            best_performance_spec = -10
            patience_auc, patience_ap, patience_spec = max_patience, max_patience, max_patience
        elif not (stopping is None):  # use EO criterion
        ...
        #For tracking progress/stopping if using VAL + SPEC
        true_spec_gap = utils.specGap(A_orig.todense())
        if not os.path.isdir(save_directory):
            os.makedirs(save_directory)

        '''MODIFIED RESTORE HERE'''
        ...
        if model_name is None:
            ...
            save_file = "{}/{}.ckpt".format(save_directory, model_name)
        ...
        if not continue_training:
            ...
            print("**** Continuing training initializing weights from saved file. ****")
        ...
        for _it in range(start_iter,max_iters):
            ...
            if _it > 0 and _it % eval_every == 0:
                ...
                M = utils.score_matrix_from_random_walks(np.array(smpls).reshape([-1, self.rw_len]), self.N)
                M = M.tocsr()


                # Assemble a graph from the score matrix
                _graph = utils.graph_from_scores(M, A_orig.sum())
                ...
                edge_scores = np.append(M[tuple(val_ones.T)].A1, M[tuple(val_zeros.T)].A1)

                auc, ap = roc_auc_score(actual_labels_val, edge_scores), average_precision_score(actual_labels_val, edge_scores)

                # Compute Validation ROC-AUC and average precision scores.
                val_performances.append((auc,ap))

                # Compute absolute distance between true spec gap and current model spec gap
                scaled_M = utils.normMatrix_wsum(M.todense(),A_orig.sum())
                spec_score = np.abs(utils.specGap(scaled_M) - true_spec_gap)

                # Update Gumbel temperature
                temperature = np.maximum(self.params['temp_start'] * np.exp(-(1-self.params['temperature_decay']) * _it),
                                         self.params['min_temperature'])

                print("**** Iter {:<6} Val ROC {:.3f}, AP: {:.3f}, EO {:.3f}, SPEC GAP DIST {:.3f} ****".format(_it,
                                                                               val_performances[-1][0],
                                                                               val_performances[-1][1],
                                                                               edge_overlap/A_orig.sum(),
                                                                               spec_score))

                if not stopping is None:
                    print('USING STOPPING')
                    if stopping == -1:   # Evaluate VAL criterion
                        if np.sum(val_performances[-1]) > best_performance_val:
                            # New "best" model
                            best_performance_val = np.sum(val_performances[-1])
                            patience_val = max_patience
                            _ = saver.save(self.session, save_file)
                        else:
                            patience_val -= 1
                        if patience_val == 0:
                            ...
                            best_iter = _it - (eval_every*max_patience)
                            samples_best = np.loadtxt(path+'/samples_{}.txt'.format(best_iter))
                            samples.savetxt(path+'/auc+ap_stop_model_samples.txt')
                            break
                    elif stopping == -2:
                        if auc > best_performance_auc:
                            best_performance_auc = auc
                            patience_auc = max_patience
                        elif patience_auc > 0:
                            patience_auc -= 1
                        if ap > best_performance_ap:
                            best_performance_ap = ap
                            patience_ap = max_patience
                        elif patience_ap > 0:
                            patience_ap -= 1
                        #minimizing spectrum score
                        if -1*(spec_score) > best_performance_spec:
                            best_performance_spec = -1*spec_score
                            patience_spec = max_patience
                        elif patience_spec > 0:
                            patience_spec -= 1
                        if (patience_spec + patience_auc + patience_ap) == 0:
                            print("**** EARLY STOPPING AFTER {} ITERATIONS ****".format(_it))
                            best_iter = _it - (eval_every*max_patience)
                            samples_best = np.loadtxt(path+'/samples_{}.txt'.format(best_iter))
                            samples.savetxt(path+'/spec+ep_stop_model_samples.txt')
                        else:
                            print("best auc {} best ap {} best spec {}".format(best_performance_ap, best_performance_auc, best_performance_spec))
                        
                    elif edge_overlap/A_orig.sum() >= stopping:   # Evaluate EO criterion
                        print("**** EARLY STOPPING AFTER {} ITERATIONS ****".format(_it))
                        samples.savetxt(path+'/eo_{}_stop_model_samples.txt'.format(stopping), M.todense())
                        break
                    
            if plot_every > 0 and _it % plot_every == 0:
                if _it > 0:
                    np.savetxt(path+'/samples_{}.txt'.format(_it),M.todense())
                    np.savetxt(path+'/disc_losses_{}_{}.txt'.format(start_iter,max_iters),disc_losses)
                    np.savetxt(path+'/gen_losses_{}_{}.txt'.format(start_iter,max_iters),gen_losses)
        #### Training completed.
